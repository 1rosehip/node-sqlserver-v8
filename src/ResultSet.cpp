...
namespace mssql
{
	...
	Local<Object> ResultSet::get_entry(const nodeTypeFactory & fact, const ColumnDefinition & definition)  {
		...
		entry->Set(context, fact.from_two_byte(L"size"), fact.new_integer(static_cast<int32_t>(definition.columnSize)));
		entry->Set(context, fact.from_two_byte(L"name"), fact.from_two_byte(definition.name.c_str()));
		entry->Set(context, fact.from_two_byte(L"nullable"), fact.new_boolean(definition.nullable != 0));
		entry->Set(context, fact.from_two_byte(L"type"), fact.from_two_byte(type_name));
		entry->Set(context, fact.from_two_byte(L"sqlType"), fact.from_two_byte(definition.dataTypeName.c_str()));
		if (definition.dataType == SQL_SS_UDT) {
			entry->Set(context, fact.from_two_byte(L"udtType"), fact.from_two_byte(definition.udtTypeName.c_str()));
		}
		return entry;
	}
	...
	Local<Value> ResultSet::meta_to_value()
    {
	   ...
	   const auto context = fact.isolate->GetCurrentContext();
	   for_each(this->_metadata.begin(), this->_metadata.end(), [fact, metadata, context](const ColumnDefinition & definition) {
		  metadata->Set(context, metadata->Length(), get_entry(fact, definition));
	   });
